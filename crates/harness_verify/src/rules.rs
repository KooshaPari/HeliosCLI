...
pub async fn run_security_rule(scanner: &str, critical_only: bool, spec_id: &str) -> Result<VerificationResult> {
    let scanner_clean = scanner.trim();
    if scanner_clean.is_empty() {
        return Ok(make_failed_result(spec_id, crate::result::VerificationType::Security, "Security scanner name is empty"));
    }
    if has_shell_metacharacters(scanner_clean) {
        return Ok(make_failed_result(spec_id, crate::result::VerificationType::Security,
    ...
    let result = match scanner_path {
        Some(path) => {
            ...
            match output {
                Ok(o) => {
                    let duration = chrono::Utc::now().signed_duration_since(start).num_milliseconds() as u64;
                    let passed = o.status.success();
                    VerificationResult {
                        id: uuid::Uuid::new_v4(),
                        spec_id: spec_id.to_string(),
                        verification_type: crate::result::VerificationType::Security,
                        status: if passed { crate::result::VerificationStatus::Passed } else { crate::result::VerificationStatus::Failed },
                        started_at: start, completed_at: Some(chrono::Utc::now()), duration_ms: duration,
                        output: format!("Security scanner '{}' found at {} (critical_only={})", scanner_clean, path, critical_only),
                        errors: if !passed { vec![String::from_utf8_lossy(&o.stderr).to_string()] } else { vec![] },
                        metrics: Default::default(),
                    }
                }
                Err(e) => VerificationResult {
                    id: uuid::Uuid::new_v4(), spec_id: spec_id.to_string(),
                    verification_type: crate::result::VerificationType::Security,
                    status: crate::result::VerificationStatus::Failed,
                    started_at: start, completed_at: Some(chrono::Utc::now()), duration_ms: 0,
                    output: format!("Security scanner '{}' failed to execute: {}", scanner_clean, e),
                    errors: vec![e.to_string()], metrics: Default::default(),
                },
            }
        }
        None => VerificationResult {
            ...
            started_at: start, completed_at: Some(chrono::Utc::now()), duration_ms: 0,
            output: format!("Security scanner '{}' not found on PATH (critical_only={})", scanner_clean, critical_only),
            errors: vec![], metrics: Default::default(),
        },
    };
    Ok(result)
}
...
pub async fn run_performance_rule(metric: &str, threshold: &str, spec_id: &str) -> Result<VerificationResult> {
        ...
        Ok(Ok(output)) => {
            ...
            let (passed, output_msg) = if let Some(actual_ns) = measured_ns {
                if let Some(max_ns) = threshold_ns {
                    ...
                    (passed, format!("Performance: {} measured {}ns, threshold {}ns -- {}", metric, actual_ns, max_ns, if passed { "PASSED" } else { "EXCEEDED" }))
                } else {
                    (true, format!("Performance: {} measured {}ns (no threshold)", metric, actual_ns))
                }
            } else {
                let wall_ns = elapsed * 1_000_000;
                if let Some(max_ns) = threshold_ns {
                    let passed = wall_ns <= max_ns;
                    (passed, format!("Performance: {} wall-clock {}ms, threshold {}ms -- {}", metric, elapsed, max_ns / 1_000_000, if passed { "PASSED" } else { "EXCEEDED" }))
                } else {
            ...
            Ok(VerificationResult {
                ...
                status: if passed { crate::result::VerificationStatus::Passed } else { crate::result::VerificationStatus::Failed },
                started_at: start, completed_at: Some(chrono::Utc::now()), duration_ms: elapsed,
                output: output_msg,
                errors: if !passed { vec![format!("Performance threshold exceeded for '{}'", metric)] } else { vec![] },
                metrics: Default::default(),
            })
        }
        _ => Ok(make_failed_result(spec_id, crate::result::VerificationType::Performance,
            &format!("Performance benchmark '{}' failed to execute", metric))),
    }
}

/// Run a custom command verification rule.
pub async fn run_custom_rule(command: &str, expected_exit_code: i32, spec_id: &str) -> Result<VerificationResult> {
    if has_shell_metacharacters(command) {
        return Ok(make_failed_result(spec_id, crate::result::VerificationType::Custom,
    ...
    Ok(VerificationResult {
        ...
        status: if passed { crate::result::VerificationStatus::Passed } else { crate::result::VerificationStatus::Failed },
        started_at: chrono::Utc::now(), completed_at: Some(chrono::Utc::now()), duration_ms: 0,
...
fn make_failed_result(spec_id: &str, vtype: crate::result::VerificationType, output: &str) -> VerificationResult {
    VerificationResult {
        id: uuid::Uuid::new_v4(), spec_id: spec_id.to_string(),
        verification_type: vtype, status: crate::result::VerificationStatus::Failed,
        started_at: chrono::Utc::now(), completed_at: Some(chrono::Utc::now()), duration_ms: 0,
        output: output.to_string(), errors: vec![], metrics: Default::default(),
    }
}
...
mod tests {
    ...
    async fn performance_rule_runs_benchmark_and_checks_threshold() {
        let result = run_performance_rule("nonexistent_benchmark_xyz", "300s", "perf-demo").await.unwrap();
